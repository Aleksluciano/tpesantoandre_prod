...
Circ: *${j.congregation.circuit}*\n`
        ...
        /*         console.log("SOCKET EMAIL");
                socket.emit('grabEscala', {
                header: `Evento Socket emitido pelo servidor por ${escala.pontos[p][u].pubs[s].firstName}`,
                idescala: req.body._id,
                p: p,
                u: u,
                s: s,
                sim: sim,
      ...
      for (let p = 0; p < escala[i].pontos.length; p++) {
        ...
        for (let u = 0; u < escala[i].pontos[p].length; u++) {
          ...
          for (let s = 0; s < escala[i].pontos[p][u].pubs.length; s++) {
            ...
            if (!escala[i].pontos[p][u].pubs[s].email) continue;
...
//console.log(text);
      ...
      /*             {
                    filename: 'img_ponto.png',
                    path: __dirname + '/pictures/img_ponto.png',
